...
class bigModelCustomRobertaForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels1 = config.num_labels1
        self.num_labels2 = config.num_labels2
        ...
        self.roberta1 = RobertaModel(config, add_pooling_layer=False)
        self.roberta2 = RobertaModel(config, add_pooling_layer=False)
        self.roberta3 = RobertaModel(config, add_pooling_layer=False)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dropout = nn.Dropout(classifier_dropout)
    ...
    def forward(
        ...
        pooled_output1 = pooled_output1[:, 0, :]
        
        output1 = self.dropout(pooled_output1)
        output1 = self.dense1(pooled_output1)
        output1 = torch.tanh(output1)
        output1 = self.dropout(output1)

        
        pooled_output2 = outputs2[0]
        pooled_output2 = pooled_output2[:, 0, :]

        output2 = self.dropout(pooled_output2)
        output2 = self.dense2(pooled_output2)
        output2 = torch.tanh(output2)
        output2 = self.dropout(output2)

        
        pooled_output3 = outputs3[0]
        pooled_output3 = pooled_output3[:, 0, :]

        output3 = self.dropout(pooled_output3)
        output3 = self.dense3(pooled_output3)
        output3 = torch.tanh(output3)
        output3 = self.dropout(output3)
        ...
        if loss1 and loss2 and loss3:
            loss = loss1 + loss2 + loss3
        if not return_dict:
            output = (logits1,) + (logits2,) + (logits3,) + outputs1
            return ((loss,) + output) if loss is not None else output
        return SequenceClassifierOutput(
            loss=loss,
            logits=(logits1,) + (logits2,) + (logits3,),
...
class addLayerCustomRobertaForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels1 = config.num_labels1
        self.num_labels2 = config.num_labels2
        ...
        self.dense1_1 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense2_1 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense3_1 = nn.Linear(config.hidden_size, config.hidden_size)

        self.dense1_2 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense2_2 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense3_2 = nn.Linear(config.hidden_size, config.hidden_size)

        self.dense1_3 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense2_3 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense3_3 = nn.Linear(config.hidden_size, config.hidden_size)

        self.dense1_4 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense2_4 = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense3_4 = nn.Linear(config.hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        output1 = self.dropout(pooled_output)
        # output1 = self.dense1_1(output1)
        # output1 = torch.tanh(output1)

        # output1 = self.dense1_2(output1)
        # output1 = torch.tanh(output1)

        # output1 = self.dense1_3(output1)
        # output1 = torch.tanh(output1)
        # output1 = self.dropout(output1)

        # output1 = self.dense1_4(output1)
        # output1 = torch.tanh(output1)
        # output1 = self.dropout(output1)
        ...
        output2 = self.dropout(pooled_output)
        # output2 = self.dense2_1(output2)
        # output2 = torch.tanh(output2)

        # output2 = self.dense2_2(output2)
        # output2 = torch.tanh(output2)

        # output2 = self.dense2_3(output2)
        # output2 = torch.tanh(output2)
        # output2 = self.dropout(output2)

        # output2 = self.dense2_4(output2)
        # output2 = torch.tanh(output2)
        # output2 = self.dropout(output2)
        ...
        output3 = self.dropout(pooled_output)
        # output3 = self.dense3_1(output3)
        # output3 = torch.tanh(output3)

        # output3 = self.dense3_2(output3)
        # output3 = torch.tanh(output3)

        # output3 = self.dense3_3(output3)
        # output3 = torch.tanh(output3)
        # output3 = self.dropout(output3)

        # output3 = self.dense3_4(output3)
        # output3 = torch.tanh(output3)
        # output3 = self.dropout(output3)
